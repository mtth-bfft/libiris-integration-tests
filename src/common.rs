...
// Don't implement a standard Drop trait on a struct, we want precise
// control over when the tmp file is cleaned up between different panic()s
...
pub fn get_worker_bin_path() -> CString {
    ...
    let test_name = exe.file_stem().unwrap().to_str().unwrap().rsplitn(2, '-').last().unwrap();
    ...
    loop {
        dir = dir.parent().expect(&format!("worker binary {} not found in any parent directory of {}", &file_name, exe.to_string_lossy())).to_path_buf();
